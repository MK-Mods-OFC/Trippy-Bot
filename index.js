...
async function starts() {
	...
	fs.existsSync('./Trippy.json') && Lxa.loadAuthInfo('./Trippy.json')
	Lxa.on('connecting', () => {
		start('2', 'Connecting...')
	})
	Lxa.on('open', () => {
	...
	await Lxa.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./Trippy.json', JSON.stringify(Lxa.base64EncodedAuthInfo(), null, '\t'))
...
const isQuotedSticker = type === 'extendedTextMessage' && content.includes('stickerMessage')
      ...
      colors = ['red','white','black','blue','yellow','green']
			...
			function addMetadata(packname, author) {	
				if (!packname) packname = 'Trippy'; if (!author) author = 'Bot';	
...
const checkLimit = (sender) => {
     ...
     if (limitCounts <= 0) return Lxa.sendMessage(from, `Lo sentimos, su límite se ha agotado, repita la compra`, text,{ quoted: mek})
      Lxa.sendMessage(from, `〘  *LIMITE* 〙
Tu límite restante : *${limitCounts}*
_El uso de límites solo se aplica a determinadas funciones._`, text, { quoted : mek})
      found = true
                                }
                        }
      if (found === false) {
...
//--Convert stiker to image
				case 'toimg':
					...
					exec(`ffmpeg -i ${medimg} ${ran}`, (err) => {
						...
						buffer = fs.readFileSync(ran)
						Lxa.sendMessage(from, buffer, image, {quoted: mek, caption: 'Trippy Bot'})
						fs.unlinkSync(ran)
					})
					break
